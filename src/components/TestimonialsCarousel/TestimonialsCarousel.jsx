...
function TestimonialsCarousel() {
  ...
  const isMobile = typeof window !== 'undefined' && window.matchMedia('(max-width: 968px)').matches;
  ...
  // Calculate card width for slider (80vw + 16px gap on mobile, 350px + 16px gap on desktop)
  const cardWidth = isMobile ? (window.innerWidth * 0.8 + 16) : (350 + 16);
  return (
    <section className="testimonials">
      <div className="container">
        <div className="testimonials__wrapper">
          {/* Desktop layout: nav left, cards, nav right */}
          {!isMobile && (
            <>
              <button 
          ...
          {/* Mobile layout: cards above, navs below */}
          {isMobile && (
            <>
              <div className="testimonials__content">
                <div 
                  className="testimonials__slider" 
                  style={{
                    transform: `translateX(calc(-${currentIndex * cardWidth}px + ${(window.innerWidth - window.innerWidth * 0.8) / 2}px))`,
                    transition: isAnimating ? 'transform 0.6s cubic-bezier(0.25, 0.46, 0.45, 0.94)' : 'none'
                  }}
                >
                  {testimonials.map((testimonial) => (
              ...
              <div className="testimonials__navs-mobile">
                <button 
                  className="testimonials__nav testimonials__nav--prev" 
                  onClick={prevTestimonial}
                  aria-label="Previous testimonials"
